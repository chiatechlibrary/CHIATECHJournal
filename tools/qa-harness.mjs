...
export function createHarness() {
  const tables = Object.fromEntries(['Users','Reviews','Articles','BlogPosts','Settings','Audit','ContentPages'].map(name => [name, []]));
  const cache = new Map();
  const mail = [];
  const properties = new Map([['PASSWORD_PEPPER','LOCAL-QA-ONLY-NOT-A-PRODUCTION-SECRET'],['PUBLIC_BASE_URL','https://journal.chiatechsolutions.com'],['ADMIN_EMAIL','qa-admin@example.invalid']]);
  const context = vm.createContext({ console, Date, JSON, CacheService: { getScriptCache: () => ({ get: key => cache.get(key) || null, put: (key, value) => cache.set(key,value), remove: key => cache.delete(key) }) },
    ...
    Utilities: { getUuid: () => crypto.randomUUID(), DigestAlgorithm: { SHA_256: 'sha256' }, Charset: { UTF_8: 'utf8' }, computeDigest: (_, value) => [...crypto.createHash('sha256').update(value).digest()], base64Encode: value => Buffer.from(value).toString('base64'), base64EncodeWebSafe: value => Buffer.from(value).toString('base64url') },
  ...
  function login(role='ADMIN') { const result = context.login({ email: role === 'ADMIN' ? admin.email : editor.email, password }); if (!result.ok) throw new Error('Local fixture login failed'); return result.token; }
