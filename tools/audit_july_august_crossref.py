...
BASE='https://journal.chiatechsolutions.com'; PREFIX='10.68232'; SCHEMA='5.5.0'
ABSTRACT_HOLDS={'e006':'TRUNCATED_CONCLUSION_REQUIRES_AUTHORITATIVE_VOR_RECOVERY','e007':'TRUNCATED_CONCLUSION_REQUIRES_AUTHORITATIVE_VOR_RECOVERY','e008':'TRUNCATED_CONCLUSION_REQUIRES_AUTHORITATIVE_VOR_RECOVERY','e020':'REBUILD_FROM_FINAL_MANUSCRIPT_AND_VERIFY_AGAINST_VOR'}
MANUAL_AUTHORS={
...
for eid in [f'e{i:03d}' for i in range(1,59)]:
 ...
 doi=f'{PREFIX}/cj.{eid}'
 abstract_status=ABSTRACT_HOLDS.get(eid,'READY_FOR_EDITORIAL_CONFIRMATION')
 if eid in ABSTRACT_HOLDS: issues.append({'article_id':eid,'issue':abstract_status})
 records.append({'article_id':eid,'title':title,'author':author_objects,'author_source_text':source_author_text,'abstract':abstract if eid not in ABSTRACT_HOLDS else '','abstract_status':abstract_status,'keywords':keywords,'proposed_doi':doi,'doi_status':'PENDING_REGISTRATION','container_title':'CHIATECH JOURNAL','volume':'2','issue':'1','published_online':[2026,8],'URL':f'{BASE}/papers/2026_V1I1_PIONEER_JULY_AUGUST_PART2/{eid}/html/{eid}.html','source_package':source})

payload={'schema':'CHIATECH_INTERNAL_CROSSREF_DEPOSIT_STAGING_V2','crossref_schema':SCHEMA,'member':'CHIA TECH SOLUTIONS AND RESOURCES LIMITED','prefix':PREFIX,'journal_title':'CHIATECH JOURNAL','volume':'2','issue':'1','publication_month':'2026-08','doi_status':'PENDING_REGISTRATION','issn_print':'PENDING_ASSIGNMENT','issn_online':'PENDING_ASSIGNMENT','issn_l':'PENDING_DESIGNATION_BY_ISSN_SYSTEM','records':records}
(OUT/'CROSSREF_DEPOSIT_V2I1_AUGUST_2026.json').write_text(json.dumps(payload,indent=2,ensure_ascii=False),encoding='utf-8')
(OUT/'CROSSREF_DEPOSIT_V2I1_AUGUST_2026.xml').write_text('<?xml version="1.0" encoding="UTF-8"?><doi_batch version="5.5.0" xmlns="http://www.crossref.org/schema/5.5.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.crossref.org/schema/5.5.0 https://data.crossref.org/schemas/crossref5.5.0.xsd"><head><doi_batch_id>chiatech-v2i1-20260912-r01</doi_batch_id><timestamp>20260912000000</timestamp><depositor><depositor_name>CHIA TECH SOLUTIONS AND RESOURCES LIMITED</depositor_name><email_address>chiatechlibrary@gmail.com</email_address></depositor><registrant>CHIA TECH SOLUTIONS AND RESOURCES LIMITED</registrant></head><body><journal><journal_metadata><full_title>CHIATECH JOURNAL</full_title><title>CHIATECH JOURNAL</title></journal_metadata><journal_issue><publication_date media_type="online"><month>08</month><year>2026</year></publication_date><journal_volume><volume>2</volume></journal_volume><issue>1>'+''.join(f'<journal_article publication_type="full_text"><titles><title>{html.escape(r["title"])}</title></titles><contributors>'+''.join(f'<person_name sequence="{"first" if i==0 else "additional"}" contributor_role="author"><given_name>{html.escape(a["given"])}</given_name><surname>{html.escape(a["family"])}</surname></person_name>' for i,a in enumerate(r['author']))+f'</contributors><publication_date media_type="online"><month>08</month><year>2026</year></publication_date><publisher_item><item_number item_number_type="article_number">{r["article_id"]}</item_number></publisher_item><doi_data><doi>{r["proposed_doi"]}</doi><resource>{r["URL"]}</resource></doi_data></journal_article>' for r in records if r['article_id'] not in ABSTRACT_HOLDS)+'</issue></journal_issue></journal></body></doi_batch>',encoding='utf-8')
