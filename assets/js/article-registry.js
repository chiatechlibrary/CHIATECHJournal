(() => {
  ...
  function citationFormats(article) {
    ...
    const bibFields = { title: article.title, author: authors.map(author => `${author.family}, ${author.given}`).join(' and '), journal, year, volume: article.volume, number: article.issue, pages, doi: article.doi, url: doi || `https://journal.chiatechsolutions.com${pageUrl(article)}`, abstract: article.abstract, keywords: (article.keywords || []).join(', ') };
    ...
    return {
      ...
      apa7: `${apaAuthors} (${year}). ${article.title}. ${journal}${location ? `, ${location}` : ''}.${doi ? ` ${doi}` : ''}`,
      mla9: `${mlaAuthors}. “${article.title}.” ${journal}${article.volume ? `, vol. ${article.volume}` : ''}${article.issue ? `, no. ${article.issue}` : ''}${year ? `, ${year}` : ''}${pages ? `, ${pages}` : ''}.${doi ? ` ${doi}.` : ''}`,
      chicago18: `${chicagoAuthors}. “${article.title}.” ${journal} ${volumeIssue}${year ? ` (${year})` : ''}${pages ? `: ${pages}` : ''}.${doi ? ` ${doi}.` : ''}`.replace(/\s+/g, ' '),
      harvard: `${harvardAuthors} ${year ? `(${year})` : ''} ‘${article.title}’, ${journal}${article.volume ? `, vol. ${article.volume}` : ''}${article.issue ? `, no. ${article.issue}` : ''}${pages ? `, ${pages}` : ''}.${doi ? ` Available at: ${doi}` : ''}`,
      ieee: `${ieeeAuthors}, “${article.title},” ${journal}${article.volume ? `, vol. ${article.volume}` : ''}${article.issue ? `, no. ${article.issue}` : ''}${pages ? `, ${pages}` : ''}${year ? `, ${year}` : ''}.${doi ? ` doi: ${doi.replace('https://doi.org/', '')}.` : ''}`,
      vancouver: `${vancouverAuthors}. ${article.title}. ${journal}. ${year}${article.volume ? `;${article.volume}` : ''}${article.issue ? `(${article.issue})` : ''}${pages ? `:${pages}` : ''}.${doi ? ` doi:${doi.replace('https://doi.org/', '')}.` : ''}`
    };
  }
