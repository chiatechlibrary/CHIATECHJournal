...
The repository is a valid Git checkout on `release/portal-launch-2026-07`, with `origin` set to `https://github.com/chiatechlibrary/CHIATECHJournal.git`. The release build has 95 public files and 46 HTML pages. The public article registry is exactly `[]`; no embargoed paper path is in the candidate set.
...
| Editorial backend | `backend/google-apps-script/Code.gs`, `appsscript.json`, `SETUP.md` | Persisted role-authorized operations, safe errors, strict production gates, reliable sheet headers and operator setup. |
...
Browser QA passed 54 desktop/tablet/phone route checks for headings, image alternatives and horizontal overflow. It exercised homepage, public founder, private-board routing, author guidance, citations, captions/transcript controls, admin navigation and keyboard Escape/focus return. Each current DOCX was rendered to 27 pages in total; every page was visually reviewed. The accessibility audit recorded zero high, medium or low findings for all six resources.
...
- `tools/test-release.mjs`: PASS — 26 release test groups.
...
After written deployment authorization, update the existing Apps Script deployment through **Manage deployments → Edit → New version → Deploy**; do not create a separate web-app deployment. Promote the successful Netlify preview. Check health, real administrator login/logout, invalid login, settings persistence, invitation/reissue, private editor records, draft visibility, blog lifecycle, mail routing, DNS, TLS, HTTP-to-HTTPS, CSP/HSTS/no-store headers and rollback availability. Remove/archive any deliberate test records.
...
**LOCAL PASS / CONDITIONAL GO FOR GIT RELEASE BRANCH.** The branch is suitable for the requested GitHub push after the staged audit passes. It is **not a production-launch approval**. Netlify Preview, Apps Script, live production and human sign-off remain unverified gates. Real papers remain outside this release.
